...
!--Version 1.9  revised volume for ternary collisions, only the relevant vibrational modes are considered 
!               in reactions, an improved procedure when there are multiple reactions possible for the 
!               collision pair
...
!
!--IFX 0 for plane flows, 1 for cylindrical flows or 2 for spherical flows (the axis for cyl. flows 
!        and the center of spherical flows is at x=0)
!
!--the type of the boundaries of the flowfield is set by ITYPE, the number code is
!    0 for a boundary with the stream
!    1 plane of symmetry for IFX=0, or center for IFX>0 and XB(1)=0, or mandatory for outer 
!      boundary if IVB=1
!    2 for solid surface
!    3 for vacuum
!    
...
!    9 for argon with pseudo-maxwell molecules
!   10 for argon with vhs molecules
!   11 for nitrogen - rot only
!   12 for nitrogen - vib only
!
!--IGS 0 if the initial state is a vacuum, 1 if it is the stream or reference gas and (if present) 
!        the secondary stream 
!--ISECS=0 if there is no secondary stream, 1 if there is a secondary stream that applies when x>0. 
!          for IFX=0 (XB(1) must then be negative and XB(2) must be positive),or x>XS for IFX>0.
...
!-- a molecule is moved appropriate to the minimum of CPDTM*(local mean coll time) and 
!           TPDTM*(local transit time) when its time falls half this time interval behind the flow time 
...
!--ISF 0    if the sample is reset only as a user restart option, 1 if it is reset at the end of each 
!           output interval (unsteady flow sampling)
!--FRACSAM  fraction (at the end) of the output interval in which samples are made in a flow with 
!           unsteady sampling 
!--JCD 0    if any reactions are based on rate equations (GASCODE=6 or 8 only), 1 if they are based on 
!           the Q-K reaction model
...
REAL(KIND=8), DIMENSION(2) :: FND,FTMP,FRTMP,FVTMP,VFX,VFY,TSURF,FSPEC,VSURF
...
IMPLICIT NONE
!
INTEGER :: IRUN,N,M,IADAPT,IRETREM,ISET, moveset
REAL(KIND=8) :: A 
!
...
IEAA=1   !if 0, activation energy adjustments depend on the macroscopic temperature; 
!        !if 1, collision temperature
IZV =1   !if 0, vibrational relaxation collision number depends on the macroscopic temperature; 
!        !if 1, collision temperature
!
...
TOTMOVI=TOTMOV
!
do while( ( moveset < 0 ) .or. ( moveset > 1 ) )
   write(*,*) 'Is the flow 0D? 0 - no, 1 - yes'
   read (*,*) moveset
   if( moveset == 1 ) write(*,*) '0D flow -> no move step will be computed'
   if( moveset == 0 ) write(*,*) '1D or greater flow -> move step will be computed'
end do
!
DO WHILE (FTIME < TLIM)
!
...
IF (GASCODE == 11) THEN
  WRITE (3,*) 'Nitrogen'
  CALL IDEAL_NITROGEN_ROT
END IF 
IF (GASCODE == 12) THEN
  WRITE (3,*) 'Nitrogen'
  CALL IDEAL_NITROGEN_VIB
END IF 
IF (GASCODE == 4) THEN
  WRITE (3,*) 'Real oxygen'
  CALL REAL_OXYGEN
 ...
 !           
             DO NSET=1,2
              DO KK=1,3
                ...
                VB(KK,NSET)=BB              
              END DO
              IF (ISPR(1,L) > 0) CALL SROT(L,FRTMP(K),ROTE(NSET))
            END DO
            A=(0.5D00*SP(5,L)*(VB(1,1)**2+VB(2,1)**2+VB(3,1)**2)+ROTE(1))/(0.5D00*BOLTZ*FRTMP(K))-3.D00-DFLOAT(ISPR(1,L))
            B=(0.5D00*SP(5,L)*(VB(1,2)**2+VB(2,2)**2+VB(3,2)**2)+ROTE(2))/(0.5D00*BOLTZ*FRTMP(K))-3.D00-DFLOAT(ISPR(1,L))
            IF (A < B) THEN
              IF (DENG < 0.D00) THEN
                KN=2
              ELSE
...
CALL ALLOCATE_GAS
!
!--values changed to Argon values
SP(1,1)=4.17D-10    !reference diameter
SP(2,1)=273.15      !reference temperature
SP(3,1)=0.5         !viscosity-temperature index
SP(4,1)=1.          !reciprocal of VSS scattering parameter (1 for VHS)
SP(5,1)=6.63D-26    !mass
...
subroutine pm_argon
  ...
  SP(1,1)=4.17D-10    !reference diameter
  SP(2,1)=273.15      !reference temperature
  SP(3,1)=1.          !viscosity-temperature index
  SP(4,1)=1.          !reciprocal of VSS scattering parameter (1 for VHS)
  SP(5,1)=6.63D-26    !mass
  ISPR(1,1)=0         !number of rotational degrees of freedom

  return
end subroutine pm_argon
!
!***************************************************************************
!
SUBROUTINE ARGON
...
!
RETURN
END SUBROUTINE IDEAL_NITROGEN_ROT
!
!***************************************************************************
!
SUBROUTINE IDEAL_NITROGEN_VIB
!
USE GAS
USE CALC
!
...
SPVM(2,1,1)=50.D0
SPVM(3,1,1)=-1
SPVM(4,1,1)=113500.D0
ISPVM(1,1,1)=4
ISPVM(2,1,1)=4
!
RETURN
END SUBROUTINE IDEAL_NITROGEN_VIB
!
!***************************************************************************
!
...
!
101 CONTINUE
OPEN (7,FILE='PARAMETERS.DAT',FORM='unformatted',ERR=101)
...
!
102 CONTINUE
OPEN (7,FILE='RESTART.DAT',FORM='unformatted',ERR=102)
READ (7) AC,AE,AVDTM,BC,BOLTZ,CCELL,CELL,CI,CLSEP,COLLS,    &
         ...
         FTMP,FTIME,FRTMP,FVTMP,GASCODE,ICCELL,ICELL,ICREF,IDISTS,IFX,IMTS,IPCELL,IPCP,     &
...
!          
          IF (ISPR(1,L) > 0) CALL SROT(L,FRTMP(KK),PROT(NM))
!          
          IF (MMVM > 0) THEN
            DO K=1,ISPV(L)
...
OPEN (7,FILE='EXTMOLS.SCR',FORM='unformatted')
...
!
  ...
  IF (SUM(1) > 0.5) THEN
    ...
    IF (SUM(1) > 1) THEN
      VAR(8,N)=(ABS(SUM(6)-SUM(2)*UU))/(3.D00*BOLTZ*SUM(1))       !--translational temperature
...
!
    ...
    VAR(11,N)=(3.*VAR(8,N)+(SUM(9)/SUM(1))*VAR(9,N))/DOF  !--overall temperature
    VAR(18,N)=VAR(3,N)*BOLTZ*VAR(8,N) !--scalar pressure (now (from V3) based on the translational temperature)
    IF (MMVM > 0) THEN
      DO L=1,MSP
        SVDF=0. !--SVDF is the sum of the degrees of freedom of species L 
        VDOF(L)=0.
        IF (ISPV(L).GT.0) THEN
          DO K=1,ISPV(L)
...
IF ((JCD == 1).AND.(MSP > 1)) THEN
  ...
  WRITE (3,*) 'TOTALS'
  DO M=1,MSP
    WRITE (3,*) ' SPECIES',M,' GAINS',TREACG(1,M)+TREACG(2,M)+TREACG(3,M)+TREACG(4,M),' LOSSES',&
         TREACL(1,M)+TREACL(2,M)+TREACL(3,M)+TREACL(4,M)  
  END DO
END IF
...
WRITE (3,*) 'Overall gas'
WRITE (3,998) 'Cell x coord.      Sample       Number Dens. Density      u velocity   v velocity   w velocity   &
     Trans. Temp. Rot. Temp.   Vib. Temp.   Temperature  Mach no.     Mols/cell    m.c.t        m.f.p        &
     mcs/mfp        speed      Pressure      TTX         TTY          TTZ   Species Fractions '
DO N=1,NCELLS
...
DO L=1,MSP
  WRITE (3,*) 'Species',L
  WRITE (3,997) 'Cell x coord.      Sample       Percentage   Species TTx   Species TTy  Species TTz  Trans. Temp.  &
       Rot. Temp.   Vib. Temp.   Spec. Temp  u Diff. Vel. v Diff. Vel. w. Diff Vel.' 
  DO N=1,NCELLS
...
!
write(7,'(a)') 'title = "dsmc profile"'
write(7,'(a)') 'variables = "Time", "Location", "Density", "Trans. Temp.", "Rot. Temp", "Vib. Temp.", &
     "Pressure", "X-Velocity", "Mach Number"'
write(7,'(a,i5)') 'zone i=',NCELLS
995 FORMAT (9G13.5)
DO N=1,NCELLS
  WRITE (7,995) 1.0d0, VAR(1,N),VAR(3,N),VAR(11,N),VAR(9,N),VAR(10,N),VAR(18,N),VAR(5,N),&
       VAR(12,N)
END DO
DO L=1,MSP
   write(7,'(a,i5)') 'zone i=',NCELLS
   DO N=1,NCELLS
      WRITE (7,995) 1.0d0, VAR(1,N),VARSP(1,N,L)*VAR(3,N),VARSP(5,N,L),VARSP(6,N,L),VARSP(7,N,L),&
           VAR(18,N),VAR(5,N),VAR(12,N)
   END DO
END DO
CLOSE(7)
!
...
IMPLICIT NONE
!
INTEGER :: N,NN,M,MM,L,LL,K,KK,KT,J,I,II,III,NSP,MAXLEV,IV,NSEL,KV,LS,MS,KS,JS,IKA
INTEGER :: IIII,LZ,KL,IS,IREC,NLOOP,IA,IDISS,IE,IEX
INTEGER :: JJ,NPRI,LIMLEV,KVV,KW,ISP1,ISP2,ISP3,INIL,INIM,JI,LV,IVM,NMC,NVM,LSI,JX
INTEGER :: MOLA,KR,JKV,NSC,KKV,NUMEXR,IAX,nstep,ISWITCH
REAL(KIND=8) :: A,AA,AAA,AB,B,BB,BBB,ASEL,DTC,SEP,VR,VRR,ECT,EVIB,ECC,ZV,COLT,ERM,C,OC
REAL(KIND=8) :: SD,D,CVR,PROB,RML,RMM,ECTOT,ETI,EREC,ET2
REAL(KIND=8) :: XMIN,XMAX,WFC,CENI,CENF,VRRT,TCOLT,EA,DEN
...
!--note quantizing of COLT in the following statements
                            ...
                            ZV=(A**SPM(3,1,1))*(SPVM(3,KV,1)*&
                                 (B**(-SPM(3,1,1))))**(((A**0.3333333D00)-1.D00)/((B**0.33333D00)-1.D00))
                          ELSE
                            ZV=SPVM(2,KV,1)
...
!--consider any reactions that are specified by rate equations                
                ...
                END IF
!
                IF ((IREC == 0).AND.(IDISS == 0)) THEN   
                   !--recombined redistribution already made
                   !               
                   !--Larsen-Borgnakke serial redistribution 
                  ECT=0.5D00*SPM(1,LS,MS)*VRR
                  DO NSP=1,2
                        ...
                        DO KV=1,ISPV(KS)
                          IF ((IPVIB(KV,K) >= 0).AND.(IDISS == 0)) THEN   
                            ...
                            IF (SPVM(3,KV,KS) > 0.) THEN
                               !--note quantizing of COLT in the following statements
                              IF (IZV == 1) THEN
                                COLT=(DFLOAT(MAXLEV)*SPVM(1,KV,KS))/(3.5D00-SPM(3,KS,JS))
                                !--quantized collision temperature
                              ELSE
                                COLT=VAR(11,NN)        !--macroscopic temperature  
                              END IF  
                              B=SPVM(4,KV,KS)/SPVM(3,KV,KS)    !Tdiss/Tref          
                              A=SPVM(4,KV,KS)/COLT       !Tdiss/T
                              ZV=(A**SPM(3,KS,JS))*(SPVM(2,KV,KS)*&
                                   (B**(-SPM(3,KS,JS))))**(((A**0.3333333D00)-1.D00)/((B**0.33333D00)-1.D00))
                            ELSE
                              ZV=SPVM(2,KV,KS)
                            END IF
                            CALL RANDOM_NUMBER(RANF)
                            IF ((1.D00/ZV > RANF).OR.(IREC == 1).OR.(IEX > 0)) THEN  
...
!--the equilibrium internal distribution functions in this section are for diatomic molecules 
!  with a single vibrational mode
