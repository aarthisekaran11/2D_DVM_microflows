...
module VelocityGrid
  ...
  type VelocityGridType
     ...
     ! TODO: where do I use _zero and should I replace with _neg, _pos?
     integer :: i_neg, i_pos
     integer :: j_neg, j_pos
     integer :: k_neg, k_pos
     ...
     ! Precalculated values used in global/local searches
...
contains
  ...
  subroutine create_velocity_grid( grid, species )
    ...
    grid%div_Nxy = one / dble( grid%num_points_x * grid%num_points_y )
    grid%div_Nx  = one / dble( grid%num_points_x )
  ...
  subroutine generate_grid( grid, index )
    ...
    call find_zero_point( grid%i_zero, grid%i_pos, grid%i_neg, grid%x, min, max )
    ...
    call find_zero_point( grid%j_zero, grid%j_pos, grid%j_neg, grid%y, min, max )
    ...
    call find_zero_point( grid%k_zero, grid%k_pos, grid%k_neg, grid%z, min, max )
  ...
  ! TODO: redo this
  subroutine find_zero_point( zero_point, pos_point, neg_point, array, min, max )
    ...
    integer :: zero_point, pos_point, neg_point
    ...
    if( array(min) .gt. zero )then
       ...
       pos_point  = min
       neg_point  = -1
       write(*,*) "Velocity bounds will prevent wall reflection"
       return
    end if
    ...
    do i = min+1, max
       if( array(i) .gt. zero )then
          ...
          pos_point  = i
          neg_point  = i-1
          if( array(i-1) .gt. -double_tol )neg_point = i-2
          if( neg_point .lt. min ) write(*,*) "Velocity bounds will prevent wall reflection"
          return
       end if
    end do
  ...
  subroutine global2local_map( l, i_min, j_min, k_min, num_points_x, num_points_y, div_Nxy, div_Nx, i, j, k )

    implicit none

    integer, intent(in) :: l, i_min, j_min, k_min, num_points_x, num_points_y
    double precision, intent(in) :: div_Nxy, div_Nx

    integer, intent(out) :: i, j, k

    ! ( i_min, j_min, k_min ) is equivalent to l = 0 in this mapping scheme
    ! TODO: I should get rid of these division signs
    ! TODO: i_min, j_min, and k_min are hardcoded to 0
    k = k_min + l / ( num_points_x * num_points_y )
!!$    k = k_min + int( l * div_Nxy )
!!$
    j = j_min + l / num_points_x - ( k - k_min ) * num_points_y
!!$    j = j_min + int( l * div_Nx ) - ( k - k_min ) * num_points_y
