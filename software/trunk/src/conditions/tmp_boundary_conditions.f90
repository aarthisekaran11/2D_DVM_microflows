...
contains
  ...
  subroutine apply_boundary_conditions( phi, phi_conv, molecule, vel_grid, species )
    ...
    call create_dist_func( boundary_nodes(1), vel_grid, molecule, species )
    call create_dist_func( boundary_nodes(2), vel_grid, molecule, species )

    ! Single walls
    do nx = 2, nx_space-1
       ! Bottom wall
       ny = 1
       call set_boundary_node( phi(nx,ny), species, molecule, vel_grid, 3 )
!       call apply_boundary_FD( phi, phi_conv, species, molecule, vel_grid, nx, ny )
       ! Top wall
       ny = ny_space
       call set_boundary_node( phi(nx,ny), species, molecule, vel_grid, 4 )
!       call apply_boundary_FD( phi, phi_conv, species, molecule, vel_grid, nx, ny )
    end do
    ...
    do ny = 2, ny_space-1
       ! Left wall
       nx = 1
       call set_boundary_node( phi(nx,ny), species, molecule, vel_grid, 1 )
!       call apply_boundary_FD( phi, phi_conv, species, molecule, vel_grid, nx, ny )
       ! Right wall
       nx = nx_space
       call set_boundary_node( phi(nx,ny), species, molecule, vel_grid, 2 )
!       call apply_boundary_FD( phi, phi_conv, species, molecule, vel_grid, nx, ny )
    end do
...
!    call apply_boundary_FD( phi, phi_conv, species, molecule, vel_grid, nx, ny )
  ...
  subroutine set_boundary_node( phi, species, molecule, vel_grid, side )
    ...
    integer, intent(in) :: species, side
    integer, parameter :: left    = 1
    integer, parameter :: right   = 2
    integer, parameter :: bottom  = 3
    integer, parameter :: top     = 4
    ...
    integer :: in_min, in_max, out_min, out_max
    integer :: i, j, k
...
!TODO: do I need this
    ...
    case(left)
       ...
       in_min  = vel_grid%i_min
       in_max  = vel_grid%i_neg
       out_min = vel_grid%i_pos
       out_max = vel_grid%i_max
    case(right)
       j_min = vel_grid%j_min
       j_max = vel_grid%j_max
       in_min  = vel_grid%i_pos
       in_max  = vel_grid%i_max
       out_min = vel_grid%i_min
       out_max = vel_grid%i_neg
    case(bottom)
       i_min = vel_grid%i_min
       i_max = vel_grid%i_max
       in_min  = vel_grid%j_min
       in_max  = vel_grid%j_neg
       out_min = vel_grid%j_pos
       out_max = vel_grid%j_max
    case(top)
       i_min = vel_grid%i_min
       i_max = vel_grid%i_max
       in_min  = vel_grid%j_pos
       in_max  = vel_grid%j_max
       out_min = vel_grid%j_min
       out_max = vel_grid%j_neg
    case default
       write(*,*) "Error in set_boundary_node: value of side must be 1-4. &
            Given value is", side
       stop
    end select
    ...
    ! Left or Right Wall
    !=====================================================================================================
    if( side .eq. left .or. side .eq. right )then

       ! Set boundary node values to zero
       boundary_nodes(1)%value = zero
       if( r_modes .gt. zero ) boundary_nodes(1)%rot = zero
       if( v_modes .gt. zero ) boundary_nodes(1)%vib = zero
    ...
    ! Bottom or Top Wall
    !=====================================================================================================
    if( side .eq. bottom .or. side .eq. top )then

       ! Set boundary node values to zero
       boundary_nodes(2)%value = zero
       if( r_modes .gt. zero ) boundary_nodes(2)%rot = zero
       if( v_modes .gt. zero ) boundary_nodes(2)%vib = zero

       ! Get wall properties
       if( side .eq. bottom )then
  ...
  end subroutine set_boundary_node
...
!    end if
    ...
    if( nx .eq. 1 )then
       ...
       ! Not at left wall so boundary node gets value of cell to left
    ...
    if( nx .eq. nx_space )then
       ...
       ! Not at right wall so boundary node gets value of cell to right
