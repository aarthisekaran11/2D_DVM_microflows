...
module BoundaryConditions
  ...
  public :: set_wall_heat_flux_dump_freq

contains

  subroutine set_wall_heat_flux_dump_freq( wall_heat_flux_dump_freq_in )
    
    implicit none

    integer, intent(in) :: wall_heat_flux_dump_freq_in

    wall_heat_flux_dump_freq = wall_heat_flux_dump_freq_in
  ...
  subroutine apply_boundary_conditions( phi, phi_conv, molecule, vel_grid, species, properties, ntime )
    ...
    type(PropertiesType), dimension(:,:) :: properties
    ...
    if( mod(ntime, wall_heat_flux_dump_freq) .eq. 0 )then
       open( unit = 10, file = "left_wall.dat", status="old", position="append" )
       write(10, advance="yes", fmt="(a7,i4)" ) "zone i=", ny_space
       close(10)
       open( unit = 11, file = "right_wall.dat", status="old", position="append" )
       write(11, advance="yes", fmt="(a7,i4)" ) "zone i=", ny_space
       close(11)
       open( unit = 12, file = "bottom_wall.dat", status="old", position="append" )
       write(12, advance="yes", fmt="(a7,i4)" ) "zone i=", nx_space
       close(12)
       open( unit = 13, file = "top_wall.dat", status="old", position="append" )
       write(13, advance="yes", fmt="(a7,i4)" ) "zone i=", nx_space
       close(13)
    end if
     
    !   open( unit = 10, file = "left_wall.dat", status="old", position="append" )
    !   write(10,fmt="(5e24.14)") ny*dy + dy*one_half, wall_heat_flux
    !   close( 10 )
!    write(10,fmt="(5e24.14)") ny*dy + dy*one_half, wall_heat_flux
    ...
    nx = 1
    ny = 1
    call set_boundary_nodes( phi, phi_conv, species, molecule, vel_grid, nx, ny, properties, ntime )
  ...
  subroutine set_boundary_nodes( phi, phi_conv, species, molecule, vel_grid, nx, ny, properties, ntime )
    ...
    integer, intent(in) :: nx, ny, species, ntime
    ...
    double precision :: x_bulk, y_bulk, z_bulk
    ...
    double precision :: Csq, Cxsq, Cysq, Czsq
    double precision :: wall_heat_flux, heat_flux_in, heat_flux_out
    ...
    ! Get bulk velocities for heat flux
    x_bulk = properties(nx,ny)%x_vel(species)
    y_bulk = properties(nx,ny)%y_vel(species)
    z_bulk = properties(nx,ny)%z_vel(species)
...
!    end if
    ...
    !=====================================================================================================
    if( nx .eq. 1 )then
       if( boundary_condition_LW .eq. wall_reflection )then
...
!             end do
!          end do
          ! Add heat flux and shear stress on left wall
          ! Calculate heat flux in to wall
          if( mod(ntime, wall_heat_flux_dump_freq) .eq. 0 )then
             heat_flux_in = zero
             do k = k_min, k_max
                z = vel_grid%z(k)
                Czsq = ( z - z_bulk )*( z - z_bulk )
                !Czsq = z * z

                do j = j_min, j_max
                   y = vel_grid%y(j)
                   Cysq = ( y - y_bulk )*( y - y_bulk )
                   !Cysq = y * y 

                   do i = vel_grid%i_min, vel_grid%i_neg
                      x = vel_grid%x(i)
                      Cxsq = ( x - x_bulk )*( x - x_bulk )
                      !Cxsq = x * x 

                      Csq = Cxsq + Cysq + Czsq

                      heat_flux_in = heat_flux_in + x*phi(nx,ny)%value(i,j,k)*Csq
                   end do
                end do
             end do
             ! Calculate heat flux out of wall
             heat_flux_out = zero
             do k = k_min, k_max
                z = vel_grid%z(k)
                Czsq = ( z - w )*( z - w )
                ...
                do j = j_min, j_max
                   ...
                   do i = vel_grid%i_pos, vel_grid%i_max
                      ...
                      heat_flux_out = heat_flux_out + x*boundary_nodes(1)%value(i,j,k)*Csq
                   end do
                end do
             end do
             wall_heat_flux = one_half*mass*(heat_flux_in + heat_flux_out)
            ! Write heat flux to file
             open( unit = 10, file = "left_wall.dat", status="old", position="append" )
             write(10,fmt="(5e24.14)") ntime*dt, (ny-1)*dy + dy*one_half, wall_heat_flux
             close( 10 )
          end if


       end if
       if( boundary_condition_LW .eq. couette )then
          boundary_nodes(1)%value=phi(nx,ny)%value
       end if
    else
...
!       end do
    ...
    if( nx .eq. nx_space )then
       if( boundary_condition_RW .eq. wall_reflection )then
...
!          end do
          ...
          if( mod(ntime, wall_heat_flux_dump_freq) .eq. 0 )then
             ...
             do k = k_min, k_max
                ...
                do j = j_min, j_max
                   ...
                   do i = vel_grid%i_min, vel_grid%i_neg
                      ...
                      heat_flux_out = heat_flux_out + x*boundary_nodes(2)%value(i,j,k)*Csq
                   end do
                end do
             end do
             wall_heat_flux = one_half*mass*(heat_flux_in + heat_flux_out)
            ! Write heat flux to file
             open( unit = 11, file = "right_wall.dat", status="old", position="append" )
             write(11,fmt="(5e24.14)") ntime*dt, (ny-1)*dy + dy*one_half, wall_heat_flux
             close( 11 )
          end if


       end if
       if( boundary_condition_RW .eq. couette )then
          boundary_nodes(2)%value=phi(1,ny)%value
       end if
    else
...
!          end do
!       end do
       ! Add heat flux and shear stress on bottom wall
       ! Calculate heat flux in to wall
       if( mod(ntime, wall_heat_flux_dump_freq) .eq. 0 )then
          heat_flux_in = zero
          do k = k_min, k_max
             ...
             do j = vel_grid%j_min, vel_grid%j_neg
                ...
                do i = i_min, i_max
                   ...
                   heat_flux_in = heat_flux_in + y*phi(nx,ny)%value(i,j,k)*Csq
                end do
             end do
          end do
          ! Calculate heat flux out of wall
          ...
          do k = k_min, k_max
             ...
             do j = vel_grid%j_pos, vel_grid%j_max
                ...
                do i = i_min, i_max
                   ...
                   heat_flux_out = heat_flux_out + y*boundary_nodes(3)%value(i,j,k)*Csq
                end do
             end do
          end do
          wall_heat_flux = one_half*mass*(heat_flux_in + heat_flux_out)
         ! Write heat flux to file
          open( unit = 12, file = "bottom_wall.dat", status="old", position="append" )
          write(12,fmt="(5e24.14)") ntime*dt, (nx-1)*dx + dx*one_half, wall_heat_flux
          close( 12 )
       end if
...
!          end do
!       end do
       ! Add heat flux and shear stress on top wall
       ! Calculate heat flux in to wall
       if( mod(ntime, wall_heat_flux_dump_freq) .eq. 0 )then
          heat_flux_in = zero
          do k = k_min, k_max
          ...
          do k = k_min, k_max
             ...
             do j = vel_grid%j_min, vel_grid%j_neg
                ...
                do i = i_min, i_max
                   ...
                   heat_flux_out = heat_flux_out + y*boundary_nodes(4)%value(i,j,k)*Csq
                end do
             end do
          end do
          wall_heat_flux = one_half*mass*(heat_flux_in + heat_flux_out)
         ! Write heat flux to file
          open( unit = 13, file = "top_wall.dat", status="old", position="append" )
          write(13,fmt="(5e24.14)") ntime*dt, (nx-1)*dx + dx*one_half, wall_heat_flux
          close( 13 )
       end if
...
!    end if
    ...
    return
  end subroutine set_boundary_nodes
