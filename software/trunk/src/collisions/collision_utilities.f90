...
module CollisionUtilities
  ...
  type CumulativeDF
     ...
     double precision, allocatable, dimension(:) :: cumul_df, eq_df
  ...
  integer, parameter :: landau_teller_SV  = 3
...
contains
  ...
  subroutine compute_num_coll_pairs( num_colls, dt, Kn, densA, densB, temp, mass, coln_rms, vel_grid, n, m )
    ...
    double precision, intent(in) :: dt, Kn, densA, densB
...
!!$    if( vel_grid(m)%beta3_min .lt. beta3 ) beta3 = vel_grid(m)%beta3_min
    ...
    beta3A = vel_grid(n)%beta3_min * mass(1)**( 3.0d0/2.0d0 )
    beta3B = vel_grid(m)%beta3_min * mass(2)**( 3.0d0/2.0d0 )
    ...
    num_colls = nint( ( one_half * dt * coll_temp * densA * densB )/( Kn * rms_sq * beta3 ) )
...
!!$    call binary_search( psi, num_points, Rf*psi( num_points ), l )
    ...
    call global2local_map( l, i_min, j_min, k_min, num_points_x, num_points_y, div_Nxy, div_Nx, i, j, k )
  ...
  subroutine split_depletion( depl_frac, m_red, properties, molecule, n, m )
    ...
    double precision :: tauMW, tauP
    double precision :: Av, Bv, coll_freq, CMW
    double precision :: dof, omega_b, vib_cs
    double precision :: C1, C2

    double precision :: trn_dof
    double precision, dimension(2) :: rot_dof, vib_dof
    double precision :: diam_sq, omega

    double precision :: Zr1, Zr2, Zv1, Zv2
    double precision :: a, b, c
    double precision :: Pr1, Pr2, Pv1, Pv2

    integer, dimension(2) :: r_modes, v_modes

    ! Mixture translational temperature is used to calculate relaxation rates
    temp  = properties%mix_tr_temp
    press = properties%mix_pressure

    ! AB VHS coefficient and AB diameter are calculated as averages in the current work
    omega   = one_half * ( molecule(n)%omega + molecule(m)%omega )
    diam_sq = one_fourth * ( molecule(n)%diam + molecule(m)%diam ) * ( molecule(n)%diam + molecule(m)%diam )

    ! AB Translational degrees of freedom is based on the VHS coefficient
    ...
    if( r_modes(1) .gt. 0 )then
       ...
       ! Zr cannot be less than 3 in the current work...
    ...
    if( v_modes(1) .gt. 0 )then
       ...
       case( millikan_white )
          Av        = molecule(n)%Av * m_red**one_half * molecule(n)%theta_v**four_thirds
          Bv        = molecule(n)%Bv * m_red**0.75d0 * molecule(n)%theta_v**four_thirds - 18.42d0
          CMW       = molecule(n)%CMW
          ...
          call calculate_collision_frequency( coll_freq, diam_sq, omega, m_red, temp, properties%dens(m) )

          ! Relaxation rate
          tauMW = CMW * exp( Av * temp**( -one_third ) + Bv ) / press
          tauP  = one / ( avg_speed * vib_cs * dens )
          Zv1   = coll_freq * ( tauMW + tauP )
    ...
    if( v_modes(2) .gt. 0 )then
       ...
       case( millikan_white )
          Av        = molecule(m)%Av * m_red**one_half * molecule(m)%theta_v**four_thirds
          Bv        = molecule(m)%Bv * m_red**0.75d0 * molecule(m)%theta_v**four_thirds - 18.42d0
          CMW       = molecule(m)%CMW
          ...
          call calculate_collision_frequency( coll_freq, diam_sq, omega, m_red, temp, properties%dens(n) )

          ! Relaxation rate
          tauMW = CMW * exp( Av * temp**( -one_third ) + Bv ) / press
          tauP  = one / ( avg_speed * vib_cs * dens )
          Zv2   = coll_freq * ( tauMW + tauP )
    ...
    ! Calculate adjustments according to Haas et al
    ! Haas, Hash, Bird, Lumpkin, Hassan, Rates of thermal relaxation in direct simulation Monte Carlo methods, 
    ! Physics of Fluids, 6(6), p. 2191-2201, 1994
    if( r_modes(1) .gt. 0 .and. r_modes(2) .gt. 0 )then
       rot_dof(1) = properties%rot_dof(n)
       rot_dof(2) = properties%rot_dof(m)

       Pr1 = ( one / Zr1 ) * ( trn_dof + rot_dof(1) ) / ( trn_dof )
       Pr2 = ( one / Zr2 ) * ( trn_dof + rot_dof(2) ) / ( trn_dof )
    ...
    if( v_modes(1) .gt. 0 .and. v_modes(2) .gt. 0 )then
       vib_dof(1) = properties%vib_dof(n)
       vib_dof(2) = properties%vib_dof(m)
       
       Pv1 = ( one / Zv1 ) * ( trn_dof + vib_dof(1) ) / ( trn_dof )
       Pv2 = ( one / Zv2 ) * ( trn_dof + vib_dof(2) ) / ( trn_dof )
...
!!$    write(*,*)"Translational degrees of freedom = ", trn_dof
!!$    write(*,*)"1st Particle Rotation:  ", Zr1, rot_dof(1), ( trn_dof ) / ( trn_dof + rot_dof(1) )
!!$    write(*,*)"2nd Particle Rotation:  ", Zr2, rot_dof(2), ( trn_dof ) / ( trn_dof + rot_dof(2) )
!!$    write(*,*)"1st Particle Vibration: ", Zv1, vib_dof(1), ( trn_dof ) / ( trn_dof + vib_dof(1) )
!!$    write(*,*)"2nd Particle Vibration: ", Zv2, vib_dof(2), ( trn_dof ) / ( trn_dof + vib_dof(2) )
!!$    write(*,*)"1st Particle Rotation:  ", Zr1
!!$    write(*,*)"2nd Particle Rotation:  ", Zr2
!!$    write(*,*)"1st Particle Vibration: ", Zv1
!!$    write(*,*)"2nd Particle Vibration: ", Zv2
  ...
  subroutine normalized_array( f, array )
  ...
  subroutine absolute_normalized_array( f, array )
  ...
  subroutine one_sign_normalized_array( f, array, sgn )
  ...
  subroutine pos_neg_split_array( fpos, fneg, array )
  ...
  subroutine pick_level( l, array, N )
  ...
  subroutine compute_rot_level_change( Jp, d_E, J, g, omega, m_red, mass, levels, &
    ...
    double precision, dimension(:), intent(in) :: level_array, fr
    ...
    case( landau_teller_SV )
       E_t = one_half * m_red * g * g
       call rot_landau_teller( d_E, E_t, mass, fr(1) )
...
!!$    star_E_r = one_half*star_level*( star_level + one )*mass*theta
  ...
  subroutine rot_nonrigid_rotor_LB( Jp, E_c, J, omega, mass, theta, Y01, Y02, &
    ...
    double precision :: alpha, determinant, inside
    double precision :: l
    double precision :: energy_level, max_level, star_level, new_level
    double precision :: star_E_r, E_r
    double precision :: R, P
    ...
    determinant = Y01*Y01 + 4.0d0*E_c*Y02
    if( determinant .gt. zero )then
       inside = one - two*alpha + two*sqrt(determinant)/Y02
    else
       inside = -one
    end if
    ...
    case( homonuclear )
       ...
       if( mod((J-1),2) .eq. 0 )then
          ...
          if( inside .lt. zero )then
             max_level = dble( levels - 1 )
          else
             max_level = one_fourth*(-one + sqrt(inside))
          end if
          ...
          if( inside .lt. zero )then
             ...
             max_level = one_fourth*(-3.0d0 + sqrt(inside))
          end if
    ...
    case( heteronuclear )
       ...
       if( inside .lt. zero )then
          ...
          max_level = one_half*(-one + sqrt(inside))
       end if
  ...
  subroutine build_Et_probability_array( T, n, m, omega, vel_grid )
    ...
    type(VelocityGridType), intent(in) :: vel_grid
    double precision, intent(in) :: T, n, m, omega
    ...
    do l = 1, num_points
       ...
       do ll = 1, num_points
          ...
          call global2local_map( ll-1, i_min, j_min, k_min, num_points_x, num_points_y, &
               div_Nxy, div_Nx, ii, jj, kk )
...
!!$    PEt = PEt/PEt_max
  ...
  subroutine vib_discrete_LB( vp, E_c, v, mass, theta, modes, levels, level_array )
    ...
    integer, intent(in) :: v, modes, levels
    double precision, intent(in) :: mass, theta
    double precision, dimension(:), intent(in) :: level_array
  ...
  subroutine rot_landau_teller( dE, E_t, mass, rot_energy )

    implicit none

    double precision, intent(in) :: E_t, mass, rot_energy

    double precision :: dE

    double precision :: E_r, E_star, rot_dof

    rot_dof = two
    
    E_r = mass * rot_energy

    E_star = rot_dof * E_t / 3.0d0

    dE = E_star - E_r

    if( ( E_t - dE ) .le. zero ) dE = E_t - double_tol * E_t

    return
  end subroutine rot_landau_teller
