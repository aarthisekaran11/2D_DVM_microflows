...
contains
  ...
  subroutine create_collision_integral( vel_grid, phi, molecule )
    ...
    case( full_monte_carlo )
       call create_full_monte_carlo( vel_grid )
  ...
  subroutine compute_collision_integral( phi, delta_phi, molecule, vel_grid, properties, nx, ny, colls_array )
...
!!$    end do
    ...
    case( full_monte_carlo )
       do n = 1, num_species
          call compute_cdf_fmc( phi(n), vel_grid(n), n )
       end do
...
!!$          if( colls_array(ns,n,n) .gt. coll_limit ) new_eq_flag = .true.
          ...
          call compute_cdf_vr( phi(n), molecule(n), vel_grid(n), properties, new_eq_flag, AA, n, nx, ny )
...
!!$             end do
             ...
             call compute_cdf_vr( phi(n), molecule(n), vel_grid(n), properties, new_eq_flag, AB, n, nx, ny )
    ...
    do n = 1, num_species
       do m = n, num_species
          ...
          case( variance_reduction )
             call variance_reduction_kernel( delta_phi, phi, colls, n, m, nx, ny, coln_rms, &
                  molecule, vel_grid, properties )
          ...
          colls_array(nx,ny,n,m) = colls
          colls_array(nx,ny,m,n) = colls
