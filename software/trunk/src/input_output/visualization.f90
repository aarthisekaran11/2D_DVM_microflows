...
module Visualization
  ...
  ! Output properites
  integer, parameter :: number_properties = 27
  logical, dimension(1:number_properties) :: properties_to_output
  character(len=128), dimension(1:number_properties) :: property_names
      
  integer :: write_stress
  integer, allocatable, dimension(:) :: stress_directions
  integer :: write_moment
  integer, allocatable, dimension(:) :: moment_numbers

  double precision, allocatable, dimension(:,:,:) :: total_data, total_stress, total_moments
  double precision, allocatable, dimension(:,:,:,:) :: species_data, species_stress, species_moments
...
contains
  ...
  subroutine set_output_locations( num_output_locations_in, x_output_locations_in, y_output_locations_in )

    implicit none

    integer, intent(in) :: num_output_locations_in
    integer, dimension(:), intent(in) :: x_output_locations_in, y_output_locations_in
    ...
    call allocate_error_check( status, "x_output_locations" )

    x_output_locations = x_output_locations_in

    allocate( y_output_locations(1:num_output_locations), STAT=status )
    call allocate_error_check( status, "y_output_locations" )

    y_output_locations = y_output_locations_in
  ...
  subroutine set_properties_output_vars( write_dens_in, &
    ...
    properties_to_output(22) = int2logical( write_rot_dof_in )
    properties_to_output(23) = int2logical( write_vib_dof_in )

    ! These variable are for analytic solutions to relaxations
    properties_to_output(24) = int2logical( write_rot_temp_relax_in )
    properties_to_output(25) = int2logical( write_vib_temp_relax_in )
    properties_to_output(26) = int2logical( write_kin_temp_relax_in )

    properties_to_output(27) = int2logical( write_pressure_in )
    ...
    property_names(22) = '"Rot DOF"'
    property_names(23) = '"Vib DOF"'
    property_names(24) = '"Rot. Temp Analytic"'
    property_names(25) = '"Vib. Temp Analytic"'
    property_names(26) = '"Trans. Temp Analytic"'
    property_names(27) = '"Pressure"'

    return
  end subroutine set_properties_output_vars

  subroutine set_stress_output( write_stress_in, stress_directions_in )

    implicit none

    integer, intent(in) :: write_stress_in
    integer, dimension(:), intent(in) :: stress_directions_in

    integer :: status

    write_stress = write_stress_in

    if( write_stress .gt. 0 )then
       allocate( stress_directions(1:write_stress), STAT=status )
       call allocate_error_check( status, "stress_directions" )

       stress_directions = stress_directions_in
    end if

    return
  end subroutine set_stress_output
  ...
  subroutine initialize_visualization( vel_grid, molecule, end_time )
    ...
    if( enable_bkw_analytic_vis )then
       ...
       call create_dist_func( bkw(1,1), vel_grid(1), molecule(1), num_species )

    end if

    ! Properties data structures
    allocate( total_data( 1:nx_space, 1:ny_space, 1:number_properties ), STAT=status )
    call allocate_error_check( status, "total_data" )

    allocate( species_data( 1:num_species, 1:nx_space, 1:ny_space, 1:number_properties ), STAT=status )
    call allocate_error_check( status, "species_data" )

    if( write_stress .gt. 0 )then
       allocate( total_stress( 1:nx_space, 1:ny_space, 1:write_stress ), STAT=status )
       call allocate_error_check( status, "total_stress" )

       allocate( species_stress( 1:num_species, 1:nx_space, 1:ny_space, 1:write_stress ), STAT=status )
       call allocate_error_check( status, "species_stress" )
    end if

    if( write_moment .gt. 0 )then
       allocate( total_moments( 1:nx_space, 1:ny_space, 1:write_moment ), STAT=status )
       call allocate_error_check( status, "total_moments" )

       allocate( species_moments( 1:num_species, 1:nx_space, 1:ny_space, 1:write_moment ), STAT=status )
  ...
  subroutine write_headers( end_time )
       ...
       do i = 1, num_species
          ...
          do loc = 1, num_output_locations
             ...
             filename = trim(adjustl(df_vis_filename))//"_"//&
                  ...
                  trim(adjustl(int2char(x_output_locations(loc))))//"_"//&
                  trim(adjustl(int2char(y_output_locations(loc))))//".dat"
    ...
    if( enable_properties_vis )then
       ...
       write( file_unit, advance="no", fmt="(a)" ) 'variables = "Time", "x-Location", "y-Location"'
!       write( file_unit, advance="no", fmt="(a)" ) 'variables = "x-Location", "y-Location"'
       ...
       if( write_stress .gt. 0 )then
          do i = 1, write_stress
             write( file_unit, advance="no", fmt="(a7,i2,a1)" ) ', "Tau ',stress_directions(i),'"'
          end do
       end if
...
!
!          call generate_bkw( bkw(1,1), mass, dens, temp, x_vel, vel_grid(1), time )
!
...
!print *, "Starting df output"
    do n = 1, num_species
!       print *, "New species loop"

       r_modes = molecule(n)%rot_modes
       v_modes = molecule(n)%vib_modes

       do loc = 1, num_output_locations
!          print *, "New locations loop"

          x_dfloc = x_output_locations(loc)
          y_dfloc = y_output_locations(loc)
          ...
          dens  = properties(x_dfloc,y_dfloc)%dens(n)
          u     = properties(x_dfloc,y_dfloc)%x_vel(n)
          v     = properties(x_dfloc,y_dfloc)%y_vel(n)
          w     = properties(x_dfloc,y_dfloc)%z_vel(n)
          temp  = properties(x_dfloc,y_dfloc)%mix_temp
          ...
          r_levels = phi(n,x_dfloc,y_dfloc)%num_rot_levels
          v_levels = phi(n,x_dfloc,y_dfloc)%num_vib_levels

          num_points_x = vel_grid(n)%num_points_x
          num_points_y = vel_grid(n)%num_points_y
          num_points_z = vel_grid(n)%num_points_z
!TODO: Look at the way these are done
!      Was wrong before but current way may
!      not be the best...
          do i = i_min, i_max
             x = vel_grid(n)%x(i)
          ...
          case( oneD )
             ...
             j = j_zero
             k = k_zero
             beta_y = vel_grid(n)%beta_y(j)
             beta_z = vel_grid(n)%beta_z(k)
             do i = i_min, i_max
                x = vel_grid(n)%x(i)
                beta_x = vel_grid(n)%beta_x(i)
                ...
                if( enable_vel_df_vis )then
                   write( file_unit, advance="no", fmt="(2e24.16)" ) &
                        phi(n,x_dfloc,y_dfloc)%value(i,j,k), phi(n,x_dfloc,y_dfloc)%value(i,j,k) - df_eq
                end if
                   ...
                   do l = 1, r_levels
                      if( r_modes .gt. 0 )then
                         write( file_unit, advance="no", fmt="(e24.16)" )&
                              phi(n,x_dfloc,y_dfloc)%rot(l,i,j,k)
                         energy = energy + phi(n,x_dfloc,y_dfloc)%rot(l,i,j,k)*phi(n,x_dfloc,y_dfloc)%rot_level(l)
                      else
                ...
                if( enable_vib_df_vis )then
                   ...
                   do l = 1, v_levels
                      if( v_modes .gt. 0 )then
                         write( file_unit, advance="no", fmt="(e24.16)" )&
                              phi(n,x_dfloc,y_dfloc)%vib(l,i,j,k)
                         energy = energy + phi(n,x_dfloc,y_dfloc)%vib(l,i,j,k)*phi(n,x_dfloc,y_dfloc)%vib_level(l)
                      else
                         write( file_unit, advance="no", fmt="(e24.16)" ) zero
                      end if
                   end do
...
!  end subroutine write_vib_levels
  ...
  subroutine write_properties( time, phi, molecule, vel_grid, properties )
    ...
    integer :: nx_space, ny_space, nx, ny, np, ns, prop, i

    double precision :: delta_x, delta_y

    double precision :: x_location, y_location
    ...
    if( nx_space .gt. 1 .or. ny_space .gt. 1 )then
       write( file_unit, advance="yes", fmt="(a7,i5)" ) "zone i=",nx_space*ny_space
    end if

    x_location = 0.5*delta_x
    do nx = 1, nx_space
       y_location = 0.5*delta_y
       do ny = 1, ny_space
          write( file_unit, advance="no", fmt="(3e24.15)" ) time, x_location, y_location
!          write( file_unit, advance="no", fmt="(3e24.15)" ) x_location, y_location
!!$          if( <nx> .lt. 120 )then
!!$             delta_x = 1.8d0
!!$          else
!!$             delta_x = 0.4d0
!!$          end if
          ...
          do prop = 1, number_properties
             if( properties_to_output(prop) ) &
                  write( file_unit, advance="no", fmt="(e24.16)" ) total_data(nx,ny,prop)
          end do

          if( write_stress .gt. 0 )then
             do i = 1, write_stress
                write( file_unit, advance="no", fmt="(e24.16)" ) total_stress(nx,ny,i)
             end do
          end if

          if( write_moment .gt. 0 )then
             do i = 1, write_moment
                write( file_unit, advance="no", fmt="(e24.16)" ) total_moments(nx,ny,i)
             end do
          end if
...
!!$             end if
             ...
             do prop = 1, number_properties
                if( properties_to_output(prop) ) write( file_unit, advance="no", fmt="(e24.16)" )&
                     species_data(ns,nx,ny,prop)
             end do
             
             if( write_stress .gt. 0 )then
                do i = 1, write_stress
                   write( file_unit, advance="no", fmt="(e24.16)" ) species_stress(ns,nx,ny,i)
                end do
             end if

             if( write_moment .gt. 0 )then
                do i = 1, write_moment
                   write( file_unit, advance="no", fmt="(e24.16)" ) species_moments(ns,nx,ny,i)
                end do
             end if
...
!  end subroutine write_speed_vib_df
  ...
  subroutine get_data_to_output( phi, molecule, vel_grid, props, time )
    ...
    double precision :: T_eq, T_rot, T_vib, coll_freq, coeff, trn_dof
    ...
    do nx = 1, nx_space
       do ny = 1, ny_space
          ...
          if( properties_to_output(1) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,1) = props(nx,ny)%dens(ns)
             end do
             total_data(nx,ny,1) = props(nx,ny)%mix_dens
          end if

          ! X-Velocity
          if( properties_to_output(2) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,2) = props(nx,ny)%x_vel(ns)
             end do
             total_data(nx,ny,2) = props(nx,ny)%mix_x_vel
          end if

          ! Y-Velocity
          if( properties_to_output(3) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,3) = props(nx,ny)%y_vel(ns)
             end do
             total_data(nx,ny,3) = props(nx,ny)%mix_y_vel
          end if

          ! Z-Velocity
          if( properties_to_output(4) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,4) = props(nx,ny)%z_vel(ns)
             end do
             total_data(nx,ny,4) = props(nx,ny)%mix_z_vel
          end if
          
          ! Speed
          if( properties_to_output(5) )then
             do ns = 1, num_species
                call compute_speed( prop_value, props(nx,ny)%x_vel(ns), props(nx,ny)%y_vel(ns), props(nx,ny)%z_vel(ns) )
                species_data(ns,nx,ny,5) = prop_value
             end do
             call compute_speed( prop_value, props(nx,ny)%mix_x_vel, props(nx,ny)%mix_y_vel, props(nx,ny)%mix_z_vel )
             total_data(nx,ny,5) = prop_value
          end if
          ...
          if( properties_to_output(6) )then
             ...
             do ns = 1, num_species
                prop_value = props(nx,ny)%x_vel(ns) / sqrt( one_half * gamma * props(nx,ny)%tr_temp(ns) / mol_mass )
                species_data(ns,nx,ny,6) = prop_value
             end do
             prop_value = props(nx,ny)%mix_x_vel/sqrt( one_half * gamma * props(nx,ny)%mix_tr_temp / mol_mass )
             total_data(nx,ny,6) = prop_value
          end if

          ! Translational Temperature
          if( properties_to_output(7) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,7) = props(nx,ny)%species_tr_temp(ns)
             end do
             total_data(nx,ny,7) = props(nx,ny)%mix_tr_temp
          end if

          ! Rotational Temperature
          if( properties_to_output(8) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,8) = props(nx,ny)%rot_temp(ns)
             end do
             total_data(nx,ny,8) = props(nx,ny)%mix_rot_temp
          end if

          ! Vibrational Temperature
          if( properties_to_output(9) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,9) = props(nx,ny)%vib_temp(ns)
             end do
             total_data(nx,ny,9) = props(nx,ny)%mix_vib_temp
          end if

          ! Total Temperature
          if( properties_to_output(10) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,10) = props(nx,ny)%temp(ns)
             end do
             total_data(nx,ny,10) = props(nx,ny)%mix_temp
          end if

          ! Temperature X
          if( properties_to_output(11) )then
             do ns = 1, num_species
                call compute_directional_temperature( prop_value, props(nx,ny)%dens(ns), &
                     props(nx,ny)%x_vel(ns), x_dir, phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                species_data(ns,nx,ny,11) = prop_value
             end do
             total_data(nx,ny,11) = zero
          end if
          ...
          if( properties_to_output(12) )then
             do ns = 1, num_species
               call compute_directional_temperature( prop_value, props(nx,ny)%dens(ns), &
                     props(nx,ny)%y_vel(ns), y_dir, phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                species_data(ns,nx,ny,12) = prop_value
             end do
             total_data(nx,ny,12) = zero
          end if
          ...
          if( properties_to_output(13) )then
             do ns = 1, num_species
               call compute_directional_temperature( prop_value, props(nx,ny)%dens(ns), &
                     props(nx,ny)%z_vel(ns), z_dir, phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                species_data(ns,nx,ny,13) = prop_value
             end do
             total_data(nx,ny,13) = zero
          end if

          ! Translational Energy
          if( properties_to_output(14) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,14) = props(nx,ny)%tr_energy(ns)
             end do
             total_data(nx,ny,14) = props(nx,ny)%mix_tr_energy
          end if
          
          ! Rotational Energy
          if( properties_to_output(15) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,15) = props(nx,ny)%rot_energy(ns)
             end do
             total_data(nx,ny,15) = props(nx,ny)%mix_rot_energy
          end if

          ! Vibrational Energy
          if( properties_to_output(16) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,16) = props(nx,ny)%vib_energy(ns)
             end do
             total_data(nx,ny,16) = props(nx,ny)%mix_vib_energy
          end if

          ! Total Energy
          if( properties_to_output(17) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,17) = props(nx,ny)%total_energy(ns)
             end do
             total_data(nx,ny,17) = props(nx,ny)%mix_energy
          end if

          ! Entropy
          if( properties_to_output(18) )then
             do ns = 1, num_species
                call compute_entropy( prop_value, molecule(ns)%mass, phi(ns,nx,ny), vel_grid(ns) )
                species_data(ns,nx,ny,18) = prop_value
             end do
             total_data(nx,ny,18) = sum( species_data(:,nx,ny,18) )
          end if

          ! Heat Flux X
          if( properties_to_output(19) )then
             do ns = 1, num_species
                call compute_heat_flux( prop_value, &
                     props(nx,ny)%x_vel(ns), props(nx,ny)%y_vel(ns), props(nx,ny)%z_vel(ns), &
                     props(nx,ny)%dens(ns), x_dir, phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                species_data(ns,nx,ny,19) = prop_value
             end do
             total_data(nx,ny,19) = sum( species_data(:,nx,ny,19) )
          end if
          ...
          if( properties_to_output(20) )then
             do ns = 1, num_species
                call compute_heat_flux( prop_value, &
                     props(nx,ny)%x_vel(ns), props(nx,ny)%y_vel(ns), props(nx,ny)%z_vel(ns), &
                     props(nx,ny)%dens(ns), y_dir, phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                species_data(ns,nx,ny,20) = prop_value
             end do
             total_data(nx,ny,20) = sum( species_data(:,nx,ny,20) )
          end if
          ...
          if( properties_to_output(21) )then
             do ns = 1, num_species
                call compute_heat_flux( prop_value, &
                     props(nx,ny)%x_vel(ns), props(nx,ny)%y_vel(ns), props(nx,ny)%z_vel(ns), &
                     props(nx,ny)%dens(ns), z_dir, phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                species_data(ns,nx,ny,21) = prop_value
             end do
             total_data(nx,ny,21) = sum( species_data(:,nx,ny,21) )
          end if

          ! Rotational Degrees of Freedom
          if( properties_to_output(22) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,22) = props(nx,ny)%rot_dof(ns)
             end do
             total_data(nx,ny,22) = zero
          end if

          ! Vibrational Degrees of Freedom
          if( properties_to_output(23) )then
             do ns = 1, num_species
                species_data(ns,nx,ny,23) = props(nx,ny)%vib_dof(ns)
             end do
             total_data(nx,ny,23) = zero
          end if
          ...
          if( properties_to_output(24) )then
             if( abs( time ) .lt. double_tol )then
                do ns = 1, num_species
                   species_data(ns,nx,ny,24) = props(nx,ny)%rot_temp(ns)
                end do
             else
                do ns = 1, num_species
                   T_eq = 4.0d0 * ( props(nx,ny)%tr_energy(ns) + props(nx,ny)%rot_energy(ns) )/&
                        ( props(nx,ny)%dens(ns) * ( 3.0d0 + props(nx,ny)%rot_dof(ns) ) )
                   trn_dof = 5.0d0 - 2.0d0 * molecule(ns)%omega
                   coeff = coeff * ( trn_dof + props(nx,ny)%rot_dof(ns) ) / trn_dof
                   coll_freq = coeff * props(nx,ny)%species_tr_temp(ns)**(one - molecule(ns)%omega)
                   call get_right_wall_temp_rot( T_rot, ns )
                   prop_value = T_eq - ( T_eq - T_rot ) * exp( -coll_freq*time/( molecule(ns)%cZr ) )
                   species_data(ns,nx,ny,24) = prop_value
                end do
             end if
             total_data(nx,ny,24) = zero
          end if

          if( properties_to_output(25) )then
             if( abs( time ) .lt. double_tol )then
                do ns = 1, num_species
                   species_data(ns,nx,ny,25) = props(nx,ny)%vib_temp(ns)
                end do
             else
                do ns = 1, num_species
                   T_eq = 4.0d0 * ( props(nx,ny)%tr_energy(ns) + props(nx,ny)%vib_energy(ns) )/&
                        ( props(nx,ny)%dens(ns) * ( 3.0d0 + props(nx,ny)%vib_dof(ns) ) )
                   trn_dof = 5.0d0 - 2.0d0 * molecule(ns)%omega
                   coeff = coeff * ( trn_dof + props(nx,ny)%vib_dof(ns) ) / trn_dof
                   coll_freq = coeff * props(nx,ny)%species_tr_temp(ns)**(one - molecule(ns)%omega)
                   call get_right_wall_temp_vib( T_vib, ns )
                   prop_value = T_eq - ( T_eq - T_vib ) * exp( -coll_freq*time/( molecule(ns)%cZv ) )
                   species_data(ns,nx,ny,25) = prop_value
                end do
             end if
             total_data(nx,ny,25) = zero
          end if
          ...
          if( properties_to_output(26) )then
             if( abs( time ) .lt. double_tol )then
                do ns = 1, num_species
                   species_data(ns,nx,ny,26) = props(nx,ny)%tr_temp(ns)
                end do
             else
                do ns = 1, num_species
                   prop_value = ( 4.0d0 / 3.0d0 ) * ( props(nx,ny)%total_energy(ns) - &
                        ( props(nx,ny)%rot_dof(ns)/4.0d0 ) * species_data(ns,nx,ny,24) - &
                        ( props(nx,ny)%vib_dof(ns)/4.0d0 ) * species_data(ns,nx,ny,25)    )
                   species_data(ns,nx,ny,26) = prop_value
                end do
             end if
             total_data(nx,ny,26) = zero
          end if

          ! Pressure
          if( properties_to_output(27) )then
             do ns = 1, num_species
                prop_value = one_half * props(nx,ny)%dens(ns) * props(nx,ny)%tr_temp(ns)
                species_data(ns,nx,ny,27) = prop_value
             end do
             prop_value = one_half * props(nx,ny)%mix_dens * props(nx,ny)%mix_tr_temp
             total_data(nx,ny,27) = prop_value
          end if
          ...
          if( write_stress .gt. 0 )then
             do i = 1, write_stress
                do ns = 1, num_species
                   call compute_viscous_stress( prop_value, stress_directions(i), &
                        props(nx,ny)%x_vel(ns), props(nx,ny)%y_vel(ns), props(nx,ny)%z_vel(ns), props(nx,ny)%pressure(ns), &
                        phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                   species_stress(ns,nx,ny,i) = prop_value
                end do
                prop_value = sum( species_stress(:,nx,ny,i) )
                total_stress(nx,ny,i) = prop_value
             end do
          end if
          ...
          if( write_moment .gt. 0 )then
             do i = 1, write_moment
                do ns = 1, num_species
                   call compute_moment( prop_value, moment_numbers(i), props(nx,ny)%dens(ns), &
                        props(nx,ny)%x_vel(ns), props(nx,ny)%y_vel(ns), props(nx,ny)%z_vel(ns), props(nx,ny)%tr_temp(ns), &
                        phi(ns,nx,ny), molecule(ns), vel_grid(ns) )
                   species_moments(ns,nx,ny,i) = prop_value
                end do
                total_moments(nx,ny,i) = zero
             end do
          end if
