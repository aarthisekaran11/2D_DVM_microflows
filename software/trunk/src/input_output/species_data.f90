...
module SpeciesAndReferenceData
  ...
  type MoleculeType
     ...
     double precision :: viscosity, T_viscosity
     ...
     double precision :: vib_cs, Av, Bv, CMW
  ...
  public :: calculate_collision_frequency
...
contains
  ...
  subroutine set_molecule_constants( molecule, num_species )
    ...
    double precision :: c_ref, sigma_ref, time_ref
    ...
    call get_scaling_parameters( c_ref, sigma_ref, time_ref )
    ...
    do n = 1, num_species
       ...
       if( molecule(n)%vib_modes .gt. 0 )then
          ! TODO: this is a constant independent of species type, not sure if it should be in molecule array
          ! Millikan and White relaxation rate coefficients
          molecule(n)%Av     =  1.16d-3 * ( mass_ref / kg2amu )**0.5d0 * temp_ref
          molecule(n)%Bv     = -1.74d-5 * ( mass_ref / kg2amu )**0.75d0 * temp_ref**( 4.0d0/3.0d0 )

          ! Millikan and White scaling constant
          molecule(n)%CMW = Pa2atm / ( two * dens_ref * kb * temp_ref ) / time_ref
       ...
       molecule(n)%viscosity   = molecule(n)%viscosity * sigma_ref / ( mass_ref * c_ref )
       molecule(n)%T_viscosity = molecule(n)%T_viscosity / temp_ref
 ...
 !!$      molecule%omega = 0.791d0
!!$       molecule%omega = 0.716d0
!!$       molecule%omega = 0.685d0
!!$       molecule%omega = 0.683d0
       molecule%alpha = 1.40d0

       molecule%viscosity   = 2.75e-5
       molecule%T_viscosity = 380.0d0
!!$       molecule%viscosity   = 4.84e-5
!!$       molecule%T_viscosity = 820.0d0
!!$       molecule%viscosity   = 9.72e-5
!!$       molecule%T_viscosity = 2300.0d0
!!$       molecule%viscosity   = 1.44e-4
!!$       molecule%T_viscosity = 4100.0d0
  ...
  subroutine calculate_collision_frequency( freq, diam_sq, omega, m_red, temp, dens )

    implicit none

    double precision, intent(in) :: diam_sq, m_red, omega, temp, dens

    double precision :: freq

    double precision :: temp_scale

    ! The temperature used to calculate reference diameters
    temp_scale = temp_diam / temp_ref

    ! The collision frequency of species A with species B where density is the density of species B
    freq = ( two / sqrt(pi) ) * diam_sq * dens * sqrt( temp_scale / m_red ) &
         * ( temp / temp_scale )**( one - omega )

    return
  end subroutine calculate_collision_frequency  
