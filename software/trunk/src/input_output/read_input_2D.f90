...
!    call grvy_input_fread_double( 'boundary/acc_coeff', acc_coeff, status )
    ...
    call variable_read(args,'dens_init',dens_init,inp_line_true)
    call variable_read(args,'x_vel_init',u_init,inp_line_true)
    call variable_read(args,'y_vel_init',v_init,inp_line_true)
    call variable_read(args,'z_vel_init',w_init,inp_line_true)
    call variable_read(args,'tr_temp_init',tr_temp_init,inp_line_true)
    call variable_read(args,'rot_temp_init',rot_temp_init,inp_line_true)
    call variable_read(args,'vib_temp_init',vib_temp_init,inp_line_true)
    ...
    do n = 1, num_species 
       ...
       call set_boundary_properties( dens_LW(n), u_LW(n), v_LW(n), w_LW(n), tr_temp_LW(n), &
            dens_RW(n), u_RW(n), v_RW(n), w_RW(n), tr_temp_RW(n), dens_BW(n), u_BW(n), v_BW(n), w_BW(n), &
            tr_temp_BW(n), dens_TW(n), u_TW(n), v_TW(n), w_TW(n), tr_temp_TW(n), n )
       call set_temp_rot( rot_temp_LW(n), rot_temp_RW(n), rot_temp_BW(n), rot_temp_TW(n), n )
       call set_temp_vib( vib_temp_LW(n), vib_temp_RW(n), vib_temp_BW(n), vib_temp_TW(n), n )
    end do
