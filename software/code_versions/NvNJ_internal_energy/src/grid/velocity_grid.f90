...
module VelocityGrid
  ...
  type VelocityGridType
     ...
     double precision :: beta3_min, beta3_max, beta3_median
...
contains
  ...
  subroutine global2local_map( l, i_min, j_min, k_min, &
  ...
  subroutine local2global_map( i, j, k, i_min, j_min, k_min, &
                               num_points_x, num_points_y, l )
  ...
  subroutine find_min_max_beta3( grid )
    ...
    grid%beta3_median = one_half*( beta3_min + beta3_max )
  ...
  subroutine generate_uniform_grid( array, min, max, num_points, v_min, v_max )
    ...
    count=0
    do i = min, max
       array(i) = v_min + dble(count)*(v_max - v_min)/dble(num_points - 1)
       count = count + 1
    end do
