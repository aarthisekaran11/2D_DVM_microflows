...
#
#   AM_PATH_GLPK( <Minimum Required Version>, <package-required=yes/no> )
#
# DESCRIPTION
#
...
#
#   On success, sets GLPK_CFLAGS, GLPK_LIBS, and #defines HAVE_GLPK. 
#   Also defines automake conditional GLPK_ENABLED.  Assumes package
...
# if test "${with_glpk}" != no ; then
    ...
    if test -d "${GLPK_PREFIX}/lib" ; then
       GLPK_LIBS="-L${GLPK_PREFIX}/lib -lglpk -Wl,-rpath,${GLPK_PREFIX}/lib"
    fi
    ...
    if test "$succeeded" = no; then
       if test "$is_package_required" = yes; then
          AC_MSG_ERROR([GLPK not found.  Try either --with-glpk or setting GLPK_DIR.])
          AC_MSG_NOTICE([optional GLPK library not found])
          GLPK_CFLAGS=""   # GRVY_CFLAGS empty on failure
          GLPK_LIBS=""     # GRVY_LIBS empty on failure
	fi
    else
	HAVE_GLPK=1
    ...
    AC_SUBST(HAVE_GLPK)
    AM_CONDITIONAL(GLPK_ENABLED,test x$HAVE_GLPK = x1)
