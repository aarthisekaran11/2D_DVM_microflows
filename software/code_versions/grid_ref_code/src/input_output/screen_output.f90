...
contains
  ...
  subroutine set_screen_output_location( screen_out_loc_in )
    ...
    integer, intent(in) :: screen_out_loc_in
    ...
    if( screen_out_loc .gt. nx_space ) screen_out_loc = nx_space
  ...
  subroutine print_property_update( ntime, num_time_steps, properties, colls_array )
    ...
    if( ntime .eq. 0 .or. mod(ntime,header_count) .eq. 0 )then
       write(*,advance="yes",fmt="(8x,8a20)") &
            "density  ", "u-velocity ", "trans. temp", &
            "rot. temp ", "vib. temp ", "energy ", "time step ", "collisions"
    end if

    dens     = properties(screen_out_loc)%mix_dens
    u        = properties(screen_out_loc)%mix_x_vel
    tr_temp  = properties(screen_out_loc)%mix_tr_temp
    rot_temp = properties(screen_out_loc)%mix_rot_temp
    vib_temp = properties(screen_out_loc)%mix_vib_temp
    energy   = properties(screen_out_loc)%mix_energy
    ...
    write(*,advance="yes",fmt="(i14)") colls_array(screen_out_loc,1,num_species)

    if( num_species .gt. 1 )then
       do n = 1, num_species

          dens     = properties(screen_out_loc)%dens(n)
          u        = properties(screen_out_loc)%x_vel(n)
          tr_temp  = properties(screen_out_loc)%tr_temp(n)
          rot_temp = properties(screen_out_loc)%rot_temp(n)
          vib_temp = properties(screen_out_loc)%vib_temp(n)
          energy   = properties(screen_out_loc)%total_energy(n)
          ...
          write(*,advance="yes",fmt="(i14)") colls_array(screen_out_loc,n,n)
