...
contains
  ...
  subroutine compute_remapping_quantities( u, v, w, f_off, vel_grid, o_i, o_j, o_k, sign_a, sign_b, sign_c, &
    ...
    if( all(uniform_grid) )then
       f_ix = ( f_off*a - f_ex*ext_x_vel ) * dble(sign_a) * vel_grid%ud_ix
       f_iy = ( f_off*b - f_ey*ext_y_vel ) * dble(sign_b) * vel_grid%ud_iy
       f_iz = ( f_off*c - f_ez*ext_z_vel ) * dble(sign_c) * vel_grid%ud_iz
    else
