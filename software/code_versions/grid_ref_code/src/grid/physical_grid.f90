...
contains
  ...
  subroutine set_physical_grid( nx_space_in, delta_x_in, ny_space_in, delta_y_in, num_zones_in )

    implicit none

    double precision, intent(in) :: delta_x_in, delta_y_in
    integer, intent(in) :: nx_space_in, ny_space_in, num_zones_in
  ...
  subroutine initialize_spatial_grid_reference( )
    ...
    allocate( spatial_grid_reference( 1:nx_space ), STAT=status )
