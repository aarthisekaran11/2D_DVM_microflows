...
if test "${with_libmesh}" != no ; then
   ...
   DEFAULT_SYS_LIB=/usr/lib
    ...
    if test -e "${LIBMESH_PREFIX}/contrib/bin/libmesh-config" ; then
	LIBMESH_CXXFLAGS=`${LIBMESH_PREFIX}/contrib/bin/libmesh-config --include`
        LIBMESH_INCLUDE=`${LIBMESH_PREFIX}/contrib/bin/libmesh-config --include`
...
###	LIBMESH_LIBS=`${LIBMESH_PREFIX}/contrib/bin/libmesh-config --ldflags`
	LIBMESH_LIBS=`${LIBMESH_PREFIX}/contrib/bin/libmesh-config --ldflags | sed s#"\-Wl,\-rpath,${DEFAULT_SYS_LIB}\ "##g `
